...
if (empty($_SESSION['username']) && empty($_SESSION['password'])){
	...
	$content .= "
				...
				<table>
					<tr>
						<td align='center' width='1000'><h4>Master Mahasiswa</h4></td>
					</tr>
				</table>
				<br>
				<table cellpadding=0 cellspacing=0 border=1>
					...
					if ($_GET['th'] == 'all'){
						$sql = $db->database_prepare("SELECT A.NIM, A.nama_mahasiswa, C.NMPSTMSPST, C.KDJENMSPST, A.gelar_depan, A.gelar_belakang, B.tahun_angkatan,
						...
						$sql = $db->database_prepare("SELECT A.NIM, A.nama_mahasiswa, A.gelar_depan, C.NMPSTMSPST, C.KDJENMSPST, A.gelar_belakang, B.tahun_angkatan,
												B.semester_angkatan, A.status_mahasiswa FROM as_mahasiswa A INNER JOIN as_angkatan B ON B.angkatan_id=A.angkatan_id
												INNER JOIN mspst C ON C.IDPSTMSPST=A.kode_program_studi
												WHERE A.angkatan_id = ?
					...
					while ($data = $db->database_fetch_array($sql)){
						if ($data['status_mahasiswa'] == 'A'){
							$status = "Aktif";
						}
						elseif ($data['status_mahasiswa'] == 'C'){
							$status = "Cuti";
						}
						elseif ($data['status_mahasiswa'] == 'D'){
							$status = "Drop-Out";
						}
						elseif ($data['status_mahasiswa'] == 'L'){
							$status = "Lulus";
						}
						elseif ($data['status_mahasiswa'] == 'K'){
							$status = "Keluar";
						}
						else{
							$status = "Non-aktif";
						...
						$content .= "
							<tr>
								...
								<td style='padding: 5px; text-align: center;'>$kd_jenjang_studi</td>
								<td style='padding: 5px; text-align: center;'>$data[NMPSTMSPST]</td>
								<td style='padding: 5px; text-align: center;'>$data[tahun_angkatan]</td>
								<td style='padding: 5px; text-align: center;'>$semester</td>
								<td align='center' style='padding: 5px; text-align: center;'>$status</td>
							</tr>
						";
						$no++;
					}
