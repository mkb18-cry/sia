...
?>
	...
	<div>
		<a href="?mod=bahan_kuliah_dosen&act=add"><button type="button" class="btn btn-green">+ Tambah Bahan/Tugas Kuliah</button></a>
	</div>
	<br>
	...
	<thead>
		<tr>
			...
			<th width='160'>Prodi</th>
			<th width='180'>Makul</th>
	...
	$sql_bahan = $db->database_prepare("SELECT * FROM as_bahan_kuliah A 
										...
										INNER JOIN mspst E ON E.IDPSTMSPST = B.prodi_id
										INNER JOIN as_kelas F ON F.kelas_id = A.kelas_id
										WHERE A.dos = ?
										GROUP BY A.bahan_id DESC")->execute($_SESSION['useri']);
	while ($data_bahan = $db->database_fetch_array($sql_bahan)){
		...
		if ($data_bahan['KDJENMSPST'] == 'A'){
			$kd_jenjang_studi = "S3";
		}
		elseif ($data_bahan['KDJENMSPST'] == 'B'){
			$kd_jenjang_studi = "S2";
		}
		elseif ($data_bahan['KDJENMSPST'] == 'C'){
			$kd_jenjang_studi = "S1";
		}
		elseif ($data_bahan['KDJENMSPST'] == 'D'){
			$kd_jenjang_studi = "D4";
		}
		elseif ($data_bahan['KDJENMSPST'] == 'E'){
			$kd_jenjang_studi = "D3";
		}
		elseif ($data_bahan['KDJENMSPST'] == 'F'){
			$kd_jenjang_studi = "D2";
		}
		elseif ($data_bahan['KDJENMSPST'] == 'G'){
			$kd_jenjang_studi = "D1";
		}
		elseif ($data_bahan['KDJENMSPST'] == 'H'){
			$kd_jenjang_studi = "Sp-1";
		}
		elseif ($data_bahan['KDJENMSPST'] == 'I'){
		...
		<tr valign=top>
			...
			<td>$kd_jenjang_studi - $data_bahan[NMPSTMSPST]</td>
			<td>$data_bahan[kode_mata_kuliah] - $data_bahan[nama_mata_kuliah_eng]</td>
			<td>$data_bahan[nama_kelas] / $data_bahan[semester]</td>
			<td>$jenis</td>
			<td>$status_bahan</td>
			<td>
			<a href='modul/mod_kuliah/download.php?id=$data_bahan[bahan_id]' title='Download'><img src='../images/download.png' width='20'></a>
			<a href='?mod=bahan_kuliah_dosen&act=edit&id=$data_bahan[bahan_id]' title='Edit'><img src='../images/edit.jpg' width='20'></a>";
			?>
				<a title="Hapus" href="modul/mod_kuliah/aksi_bahan_kuliah_dosen.php?mod=bahan_kuliah_dosen&act=delete&id=<?php echo $data_bahan[bahan_id];?>" onclick="return confirm('Anda Yakin ingin menghapus bahan/tugas kuliah <?php echo $data_bahan[judul];?>?');"><img src='../images/delete.jpg' width='20'></a>
			<?php
			echo "</td>
		</tr>";
		$no++;
...
<?php
	...
	case "add":
?>
	<a href="?mod=bahan_kuliah_dosen"><img src="../images/back.png"></a>
	<h5>Tambah Bahan/Tugas Kulaih</h5>
	<div class="box round first fullpage">
		<div class="block ">
			<form id="frm_kuliah" action="modul/mod_kuliah/aksi_bahan_kuliah_dosen.php?mod=bahan_kuliah_dosen&act=input" method="POST" enctype="multipart/form-data">
			<table class="form">
				<tr valign="top">
...
?>
	...
	<div class="box round first fullpage">
		<div class="block ">
			<form id="frm_kuliah" action="modul/mod_kuliah/aksi_bahan_kuliah_dosen.php?mod=bahan_kuliah_dosen&act=update" method="POST" enctype="multipart/form-data">
